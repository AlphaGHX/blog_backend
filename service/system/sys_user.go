...
func (s *UserService) VerifyUser(user models.User) error {
	...
	if result.RowsAffected == 0 {
		global.LOG.Errorln("UserService.VerifyUser RowsAffected == 0")
	}
	return result.Error
}
...
func (s *UserService) GetAdminInfo() (*response.GetAdminInfo, error) {
	...
	if result.Error != nil {
		global.LOG.Errorln("UserService.GetAdminInfo Find myLinks", result.Error.Error())
	...
	if result.Error != nil {
		global.LOG.Errorln("UserService.GetAdminInfo Find adminInfo", result.Error.Error())
		return nil, result.Error
	}
...
func (s *UserService) SetAdminInfo(adminInfo request.SetAdminInfo) error {
	...
	if result.Error != nil {
		global.LOG.Errorln("UserService.SetAdminInfo Save", result.Error.Error())
		return result.Error
	}

	result = global.GORM.Exec("DELETE FROM my_links")
	if result.Error != nil {
		global.LOG.Errorln("UserService.SetAdminInfo Exec", result.Error.Error())
		return result.Error
	}
